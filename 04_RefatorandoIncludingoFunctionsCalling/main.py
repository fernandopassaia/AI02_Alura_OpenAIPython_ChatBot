...
# Esse metodo contém tratativa de erro
@app.route("/chat", methods=["POST"])
def chat():
    prompt = request.json["msg"]
    response = bot(prompt)
...
def image_upload():
    global sent_image_path  # seria o "this" no C#, ele usará a variável declarada na linha 31
    print(request.files)
...
def bot(prompt):
    ...
    while True:
        try:
            ...
            if (
                sent_image_path != None
            ):  # verifico se uma imagem foi anexada, se foi, chamo o método com vision
                vision_response = analyse_image(sent_image_path)
                vision_response += (
                    ". Na resposta final, apresente detalhes da descrição da imagem."
                )
